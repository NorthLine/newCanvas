/**
 * @name jquery.centerPanZoom   居中拖拽放大缩小插件
 * @param onStart 开始时触发事件
 * @param onChange 变动时触发事件
 * @param onZoom 缩放时触发事件
 * @param onPan 拖拽时触发事件
 * @param onEnd 结束时触发事件
 * @param onReset 重置时触发的事件
 *
 * */
...
(function ($) {
    /**
     * @name jquery.centerPanZoom
     * @description 居中拖拽放大缩小插件
     * @param option 事件对象
     * @param option.onStart 开始时触发事件
     * @param option.onChange 变动时触发事件
     * @param option.onZoom 缩放时触发事件
     * @param option.onPan 拖拽时触发事件
     * @param option.onEnd 结束时触发事件
     * @param option.onReset 重置时触发的事件
     *
     * @function  reSetCenterPosition  内部容器居中
     * @function  reSetBoxWidthAndHeigt 同步内容和容器大小
     * @function  $.fn.panToObj 指定对象居中
     *
     **/
    $.fn.centerPanZoom = function (option) {
        option = option || {}
        var $this = this
        var getChindren = this.children().clone(true);
        var parentId = 'parent' + parseInt(Math.random() * 1200);
        var panZoomId = 'panZoom' + parseInt(Math.random() * 1200);
        var InnerId = 'panZoomInner' + parseInt(Math.random() * 1200);
        var templete = '<div style="width: 100%;height: 100%;" id="' + parentId +
            '">' +
            '<div style="opacity：0" id="' + panZoomId +
            '">' +
            '<div style="display: inline-block;position: absolute" id="' + InnerId +
            '">' +
            '</div>' +
            '</div>' +
            '</div>'
        this.html(templete)
        var $box = this.find('#' + panZoomId)
        var $parentBox = this.find('#' + parentId)
        var $InnerBox = this.find('#' + InnerId)
        getChindren.appendTo($InnerBox)
        $this.px = 0
        $this.py = 0
        $this.Zoom = 1

        //@name reSetCenterPosition
        //@description 内部容器居中
        this.reSetCenterPosition = function () {
            $this.Zoom=1
            $panZoom.panzoom('zoom',$this.Zoom);
            var getOuterWidth = $parentBox.width()
            var getOuterHeight = $parentBox.height()
            var getInnerWIdth = $box.width()
            var getInnerHeight = $box.height()
            $this.px = (getOuterWidth - getInnerWIdth) / 2
            $this.py = (getOuterHeight - getInnerHeight) / 2
            $panZoom.panzoom("pan", $this.px, $this.py)
        }

        //@name reSetBoxWidthAndHeigt
        //@description 同步内容和容器大小
        this.reSetBoxWidthAndHeigt = function () {
            $box.css({
        ...
        //@description 指定对象居中
        //@obj  要居中对象的Jq实例
        this.panToObj = function (obj) {
            var getBoxTop = $this.offset().top+$this.height()/2;
            var getBoxLeft = $this.offset().left+$this.width()/2;
            var getObjTop = obj.offset().top + obj.height()*$this.Zoom/2;
            var getObjLeft = obj.offset().left + obj.width()*$this.Zoom/2
            $this.px = $this.px + getBoxLeft - getObjLeft
            $this.py = $this.py + getBoxTop - getObjTop

            $panZoom.panzoom("pan", $this.px, $this.py)
        }

        // this.reSetBoxWidthAndHeigt()


        var $panZoom = $box.panzoom({
            minScale: 0.1,
            maxScale: 3,
            ...
            onChange: function (a, b, c) {
                ...
                $this.Zoom=c[0]
                if (option.onChange) {
                    option.onChange(a, b, c)
                }
            },
            onZoom: option.onZoom || function () {
            },
            onPan: option.onPan || function () {
            },
        ...
        $parentBox.on('mousewheel.focal', function (e) {
            e.preventDefault();
